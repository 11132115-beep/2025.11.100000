...
// 新增：測驗卷筆記 URL
const iframeURL3 = 'https://hackmd.io/@IPa9_G7tTe268b5mtxTumQ/rJarYtHJbg';


// 新增：淡江大學 URL
const iframeURL4 = 'https://www.tku.edu.tw/';
...
function draw() {
  ...
  // 新標籤陣列（新增 4. 測驗卷筆記，將「回到首頁」改為 5，新增 6. 淡江大學）
  const labels = [
    "1. 第一單元作品",
    "2. 第一單元講義",
    "3. 測驗系統",
...
function createPhotoButton() {
  ...
  // 嘗試與上方 loadImage 相同的路徑候選清單（會自動選到第一個可用的）
  const imgCandidates = [
    './照片.jpg',
    './照片',
    './photo.jpg/照片.jpg',
  ...
  (function trySetImg(i = 0) {
    if (i >= imgCandidates.length) {
      console.warn('圖片元素載入失敗，請確認檔名/路徑。建議放到專案根目錄並命名為 photo.jpg');
      return;
    }
    const test = new Image();
    test.onload = () => { photoImg.src = imgCandidates[i]; };
    test.onerror = () => trySetImg(i + 1);
    test.src = imgCandidates[i];
  })();
  photoImg.alt = '我的照片';
...
// 載入要在畫布上顯示的圖片（放在專案根目錄或子資料夾）
// 確認已將檔案存為 c:\Users\User\Downloads\2025.11.10\photo.jpg
// canvasPhoto = loadImage('./photo.jpg',
// () => { /* 成功 */ },
// () => { console.warn('載入 photo.jpg 失敗，請確認放置於專案資料夾並命名為 photo.jpg'); }
// );




// 嘗試多個可能路徑：你的說法是「資料夾名字是 photo.jpg、照片檔名是 照片」
// 會依序嘗試常見組合（包含有無副檔名與放在 photo.jpg 資料夾內）
...
(function tryLoadCanvas(i = 0) {
  if (i >= photoCandidates.length) {
    console.warn('載入照片失敗，請檢查檔案位置與名稱（建議放在專案根目錄並命名為 photo.jpg）');
    canvasPhoto = null;
    return;
  }
  loadImage(photoCandidates[i],
    img => { canvasPhoto = img; },
    () => { tryLoadCanvas(i + 1); }
  );
})();
